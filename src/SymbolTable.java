...
class SymbolTable {
  ...
  public Method getMethod(String id, String classScope) {
	...
	if (t1 == null || t2 == null) return true; //false;

	if (t1 instanceof BasTyp && t2 instanceof  BasTyp)
	    return true;        
	if (t1 instanceof QuaIde && t2 instanceof  QuaIde){
	    QuaIde i1 = (QuaIde)t1;
	    QuaIde i2 = (QuaIde)t2;
            Class c = getClass(i2.i1.s);
	    while(c != null) {
		if (i1.i1.s.equals(c.getId())) return true;
		else {
	...
	if (t1 instanceof ParIde && t2 instanceof  ParIde){
	    ParIde i1 = (ParIde)t1;
	    ParIde i2 = (ParIde)t2;
            Class c = getClass(i2.f0.s);
	    while(c != null) {
		if (i1.f0.s.equals(c.getId())) return true;
		else {
		    if(c.parent() == null) return false;
		    c = getClass(c.parent());
		}
	...
	if (t1 instanceof Obje && t2 instanceof Obje)
	    return true;
//	if (t1 instanceof TypNam && t2 instanceof TypNam){
//	}
	return false;	
    }
...
class Class {
	...
	parent = p;
        Iden i=new Iden(id); 
	type = new ParIde(i,null); // QuaIde(i,null);
